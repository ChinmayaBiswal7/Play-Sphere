/**
 * PlaySphere 5 — Dashboard Home Logic (Complete Rebuild)
 */
(function () {
  'use strict';
  ...
  const GAMES = [
    ...
    },
    {
      ...
      rating: '⭐ 4.9 Rating',
      dev: 'DeepMind Arcade',
      desc: 'Fast-paced 3D arcade soccer with zero rules! Physics-based ball bounce dynamics, dynamic goalkeeper diving, visual shot charging, sliding tackle dashes, and smart bot opponents.'
    },
    {
      id: 'football',
      ...
      desc: 'Next-generation 3D physics-driven football simulation. Responsive pass/shoot dynamics, spectacular curve ball VFX, player running/kicking animations, and dual remote phone controller support.'
    },
    {
      id: 'cricket',
  ...
  function selectGame(index) {
    ...
    setTimeout(() => {
      ...
      const pillEl  = document.getElementById('game-pill');
      ...
      if (devEl)    devEl.textContent    = meta.dev;
  ...
  function init() {
    ...
    window.addEventListener('keydown', e => {
      if (document.querySelector('.show') || document.activeElement.tagName === 'INPUT') return;
    ...
    if (ctrlBtn) {
      ctrlBtn.addEventListener('click', () => {
        if (typeof window.ps5OpenControllerModal === 'function') window.ps5OpenControllerModal();
      });
    }
    ...
    if (profBtn) {
      profBtn.addEventListener('click', () => {
        if (typeof window.ps5OpenProfileModal === 'function') window.ps5OpenProfileModal();
      });
    }
    ...
    // Tabs
    ['tab-games', 'tab-media'].forEach(id => {
